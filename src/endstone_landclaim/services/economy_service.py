...
class EconomyService:
    ...
    def get_currency(self) -> str:
        return str(self.config.get("economy.currency", "coins"))
    ...
    async def refund_player(
            self,
            player_uuid: str,
            amount: float,
            reason: str = "Land claim refund",
    ) -> bool:
        if not self.api:
            return False
    ...
    def calculate_claim_creation_cost(self, claim_area: int) -> float:
        base_cost = self.get_creation_cost()
        return float(base_cost)

    def calculate_claim_expansion_cost(self, old_area: int, new_area: int) -> float:
        additional_blocks = max(0, new_area - old_area)
        cost_per_block = self.get_expansion_cost_per_block()
        return float(additional_blocks * cost_per_block)

    def calculate_daily_maintenance(self, claim_area: int) -> float:
